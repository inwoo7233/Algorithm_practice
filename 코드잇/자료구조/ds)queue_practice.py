...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        if self.queue:
            print("{}님의 {} 문의 내용 접수 되었습니다. 담당자가 배정되면 {}로 연락드리겠습니다!".format(self.queue[0].name, self.queue[0].content,
                                                                           self.queue[0].email))
            del self.queue[0]
        else:
    ...
    def add_complaint(self, name, email, content):
        ...
        new_complanint = CustomerComplaint(name, email, content)
        self.queue.append(new_complanint)
        # 코드를 쓰세요
